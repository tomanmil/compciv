...
echo '6.'
cat data-hold/legislators-current.json | jq '.[] .bio .religion' | sort | uniq -c | sort -rn | tr -d '"'
echo '7.'
cat data-hold/legislators-current.json | \
...
echo '10.'
cat data-hold/legislators-current.json |   jq --raw-output '.[] .terms[-1:][0].party' | grep -v 'Republican' | grep -cv 'Democrat'
echo '11.'
cat data-hold/legislators-current.json | \
